...
class BendingDrawingFileWizard(models.TransientModel):
    ...
    por_drawing_file_ids = fields.Many2many("sale.split.pdf.page", string="POR Files", related="mmo_id.pdf_file_ids", readonly=False,
    ...
    def _compute_total_image(self):
        for record in self:
            page_ids = record.selection_file.page_ids.filtered(lambda x: not x.is_invisible_map_pcs if record.view_type == 'pcs' else not x.is_invisible_map_por)
...
class PageBendingLineWizard(models.TransientModel):
    ...
    pdf_file_ids = fields.Many2many("sale.split.pdf.page", string="Drawings", related="mmo_id.pdf_file_ids",
                                    readonly=False, domain="[('file_id', '=', selection_file), ('is_invisible_map_pcs', '=', False)]")
